...
localhost:def,ovr";
        ...
        public void ValidateRemoteServerAvailabilityTest_LocalhostConnected()
        {
            ...
            int expected = 0; 
            int actual;
            actual = RemoteExecution_Accessor.ValidateRemoteServerAvailability(remoteServers, out remoteServerData, out errorMessages);
...
anotherhost\SQLEXPRESS:SqlBuildTest,SqlBuildTest";
        ...
        public void ExecuteTest_ErrorInExecution()
        {
            ...
            Assert.AreEqual(expected, actual, "Ensure that you have your service running on \"localhost\\SQLEXPRESS\"");
            Assert.IsTrue(Directory.GetFiles(Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location), "*.cfg").Length > 0); 
        }
