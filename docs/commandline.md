...
| `--trustservercertificate` / `--trustcert` | bool | `false` | SQL Server only. When `true`, skips TLS certificate validation for non-Azure servers (`TrustServerCertificate=true`). Recognized Azure SQL endpoints always validate the certificate and ignore this bypass. Default validates the server certificate; only enable for local development or self-signed certificates you control. |
