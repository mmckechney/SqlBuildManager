...
if ([string]::IsNullOrWhiteSpace($repoRoot)) {
    # Derive from script location - this script is in /infra/scripts, so grandparent is repo root
...
if ($sqlServerDeployed -ne "false") {
    ...
    if (Test-Path $scriptPath) {
        ...
        Write-Host "  .\scripts\Database\grant_identity_permissions.ps1 -envName $envName -resourceGroupName $resourceGroupName" -ForegroundColor Yellow
    }
} else {
...
if ($pgDeployed -eq "true") {
    ...
    if (Test-Path $pgScriptPath) {
        & $pgScriptPath -envName $envName -resourceGroupName $resourceGroupName
    } else {
