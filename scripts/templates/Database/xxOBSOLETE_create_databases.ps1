...
az deployment group create --resource-group $resourceGroupName --template-file "$scriptDir/../../infra/modules/network.bicep" --parameters nsgName="$nsgName" nsgBatchName="$nsgBatchName" vnetName="$vnet" aksSubnetName="$aksSubnet" containerAppSubnetName="$containerAppSubnet" aciSubnetName="$aciSubnet" batchSubnetName="$batchSubnet" privateEndpointSubnetName="$privateEndpointSubnet" -o table
...
az deployment group create --resource-group $resourceGroupName --template-file "$scriptDir/../../infra/modules/database.bicep" --parameters envName="$envName" sqlServerBaseName="$sqlServerBaseName" sqlElasticPoolBaseName="$sqlElasticPoolBaseName" identityName="$identityName" vnetName="$vnet" testDbCountPerServer="$testDatabaseCount" currentIpAddress=$ipAddress subnetNames=$subnetNames -o table
...
.$scriptDir/grant_identity_permissions.ps1 -envName $envName -resourceGroupName $resourceGroupName -path $path
