...
if($haveSqlInfo)
{
    ...
    ..\..\src\SqlBuildManager.Console\bin\Debug\net5.0\sbm.exe batch savesettings --batchpoolname "SqlBuildManagerPoolWindows" -os "Windows" --settingsfile "$tmpPath"  --settingsfilekey "$keyFile" --username "$sqlUserName" --password "$sqlPassword" --batchaccountname "$batchAccountName" --batchaccountkey "$batchAcctKey" --batchaccounturl "https://$batchAcctEndpoint" --batchnodecount 2  --batchvmsize "STANDARD_DS1_V2" --rootloggingpath "C:/temp" --storageaccountname "$storageAccountName"  --storageaccountkey "$storageAcctKey" -eh "$eventHubConnectionString" --defaultscripttimeout 500 --concurrency 5 --concurrencytype "Count" --clientid "$($identity.clientId)" --principalid "$($identity.principalId)" --resourceid "$($identity.id)" --idrg "$($identity.resourceGroup)" --subscriptionid $subscriptionId --silent 
