...
foreach ($server in $servers)
{
    ...
    if($null -eq $exists)
    {
        ...
        $result = az sql server firewall-rule create --resource-group $resourceGroupName --server $server --name $auto --start-ip-address $ipAddress --end-ip-address $ipAddress
    }
}