                ...
                if (feature.FeatureId.ToLower() == featureKey.ToLower())
                {
                    ...
                    if (feature.Deny != null && feature.Deny.Length > 0)
                    {
                        var matchLogin = from d in feature.Deny 
                                    ...
                                    select d;
                        var matchGroup = from d in feature.Deny join g in adGroupMemberships on d.GroupName.ToLower() equals g.ToLower()
                                         select d;
                    ...
                    if (feature.Allow != null && feature.Allow.Length > 0)
                    {
                       var matchLogin = from a in feature.Allow 
                                    ...
                                    select a;
                        var matchGroup = from a in feature.Allow join g in adGroupMemberships on a.GroupName.ToLower() equals g.ToLower()
                                         select a;
