...
namespace SqlBuildManager.Console.Dependent.MySQL.UnitTest
{
    ...
    public class LocalBuildTest
    {
        ...
        /// <summary>
        /// Single DB build with explicit --server/--database targeting MySQL
        /// </summary>
        [TestMethod()]
        public async Task MySQL_LocalBuild_SingleDb_NoOverrideSetting()
        {
        ...
        /// </summary>
        [TestMethod()]
        public async Task MySQL_LocalBuild_SingleDb_SingleOverrideSetting()
        {
            Initialization init = GetInitializationObject();
            string sbmFileName = Path.GetTempPath() + Guid.NewGuid().ToString() + ".sbm";
            ...
            string[] args = (new string[] {
                ...
                "--transactional", "true",
                "--trial", "false",
                "--override", "client,sbm_mysql_test",
                "--packagename", sbmFileName,
                "--timeoutretrycount", "0"
            }).Concat(Initialization.GetAuthArgs())
              .Concat(Initialization.GetPlatformArgs())
        ...
        /// </summary>
        [TestMethod()]
        public async Task MySQL_LocalBuild_MultiDb_DoubleOverrideSetting()
        {
            Initialization init = GetInitializationObject();
            string sbmFileName = Path.GetTempPath() + Guid.NewGuid().ToString() + ".sbm";
        ...
        /// </summary>
        [TestMethod()]
        public async Task MySQL_LocalBuild_MultiDb_SingleOverrideSetting()
        {
            Initialization init = GetInitializationObject();
            string sbmFileName = Path.GetTempPath() + Guid.NewGuid().ToString() + ".sbm";
            File.WriteAllBytes(sbmFileName, Properties.Resources.PG_SimpleSelect);
        ...
        /// </summary>
        [TestMethod()]
        public async Task MySQL_LocalBuild_SingleDb_Trial()
        {
            Initialization init = GetInitializationObject();
            string sbmFileName = Path.GetTempPath() + Guid.NewGuid().ToString() + ".sbm";
            File.WriteAllBytes(sbmFileName, Properties.Resources.PG_SimpleSelect);
        ...
        /// </summary>
        [TestMethod()]
        public async Task MySQL_LocalBuild_SyntaxError_ShowsFailure()
        {
            Initialization init = GetInitializationObject();
            string sbmFileName = Path.GetTempPath() + Guid.NewGuid().ToString() + ".sbm";
