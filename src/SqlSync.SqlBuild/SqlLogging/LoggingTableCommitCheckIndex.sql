...
IF NOT  EXISTS (SELECT 1 FROM sysindexes WHERE name = 'IX_SqlBuild_Logging_CommitCheck'  AND OBJECT_NAME(id) = N'SqlBuild_Logging')
	BEGIN
		...
		PRINT 'Added [IX_SqlBuild_Logging_CommitCheck] index to SqlBuild_Logging table'
	END
ELSE
	BEGIN
		PRINT '[IX_SqlBuild_Logging_CommitCheck] index already exists in SqlBuild_Logging table'
	END
