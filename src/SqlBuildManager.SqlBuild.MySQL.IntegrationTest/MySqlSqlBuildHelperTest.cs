...
namespace SqlBuildManager.SqlBuild.MySQL.IntegrationTest
{
    /// <summary>
    /// Integration tests for the build engine running against MySQL.
    /// Requires a local MySQL instance. Configure via environment variables:
    ...
    [TestClass]
    [DoNotParallelize]
    public class MySqlSqlBuildHelperTest
    {
        ...
        private static List<MySqlInitialization> initColl = null!;
        private static bool isMySqlAvailable = false;

        [ClassInitialize]
        public static void InitializeTests(TestContext testContext)
        {
            initColl = new List<MySqlInitialization>();
            try
            {
                var server = Environment.GetEnvironmentVariable("SBM_TEST_MYSQL_SERVER") ?? "localhost";
                var user = Environment.GetEnvironmentVariable("SBM_TEST_MYSQL_USER") ?? "root";
                var password = Environment.GetEnvironmentVariable("SBM_TEST_MYSQL_PASSWORD") ?? "MySq1Adm!n";
                using var conn = new MySqlConnection($"Host={server};Database=mysql;Username={user};Password={password};Connection Timeout=5");
                conn.Open();
        ...
        private MySqlInitialization GetInitializationObject()
        {
            if (!isMySqlAvailable)
                Assert.Inconclusive("MySQL is not available.");

            var init = new MySqlInitialization();
            initColl.Add(init);
            return init;
        }
        ...
        [TestMethod]
        public async Task MySQL_RunBuildScripts_FinishCommitted()
        {
        ...
        [TestMethod]
        public async Task MySQL_RunBuildScripts_FinishRollback()
        {
            var init = GetInitializationObject();
        ...
        [TestMethod]
        public async Task MySQL_RunBuildScripts_SelectScript_Committed()
        {
            var init = GetInitializationObject();
        ...
        [TestMethod]
        public async Task MySQL_RunBuildScripts_InsertThenSelect_Committed()
        {
            var init = GetInitializationObject();
        ...
        [TestMethod]
        public void MySQL_ConnectionData_HasCorrectPlatform()
        {
            var init = GetInitializationObject();
            Assert.AreEqual(DatabasePlatform.MySQL, init.connData.DatabasePlatform);
        }

        [TestMethod]
        public void MySQL_SqlBuildHelper_UsesMySqlProviders()
        {
            var init = GetInitializationObject();
            SqlSyncBuildDataModel buildData = init.CreateSqlSyncSqlBuildDataModelObject();
            SqlBuildHelper sbh = init.CreateSqlBuildHelper(buildData);

            Assert.IsInstanceOfType(sbh.TransactionManager, typeof(MySqlTransactionManager), "Should use MySqlTransactionManager");
            Assert.IsInstanceOfType(sbh.SyntaxProvider, typeof(MySqlSyntaxProvider), "Should use MySqlSyntaxProvider");
            Assert.IsInstanceOfType(sbh.ResourceProvider, typeof(MySqlResourceProvider), "Should use MySqlResourceProvider");
        }
        ...
        [TestMethod]
        public async Task MySQL_RunBuildScripts_SingleRunScript()
        {
            var init = GetInitializationObject();
            SqlSyncBuildDataModel buildData = init.CreateSqlSyncSqlBuildDataModelObject();
        ...
        [TestMethod]
        public async Task MySQL_RunBuildScripts_SkippingPreRunScript()
        {
            var init = GetInitializationObject();
            SqlSyncBuildDataModel buildData = init.CreateSqlSyncSqlBuildDataModelObject();
        ...
        [TestMethod]
        public async Task MySQL_RunBuildScripts_TrialSuccessful()
        {
            var init = GetInitializationObject();
            SqlSyncBuildDataModel buildData = init.CreateSqlSyncSqlBuildDataModelObject();
            init.AddInsertScript(ref buildData, true);
        ...
        [TestMethod]
        public async Task MySQL_RunBuildScripts_TrialWithFailure()
        {
            var init = GetInitializationObject();
            SqlSyncBuildDataModel buildData = init.CreateSqlSyncSqlBuildDataModelObject();
            init.AddFailureScript(ref buildData, true, true);
        ...
        [TestMethod]
        public async Task MySQL_RunBuildScripts_WithFailureDontCauseFailure()
        {
            // MySQL can continue after a non-fatal script error when the error is rolled
            ...
            Assert.AreEqual(BuildItemStatus.Committed, actual.FinalStatus, "MySQL should commit non-fatal errors that are rolled back to a savepoint");
        ...
        [TestMethod]
        public async Task MySQL_RunBuildScripts_WithFailureDontRollbackDontCauseFailure()
        {
            // MySQL behavior: same as above — the aborted transaction state
            ...
            Assert.AreEqual(BuildItemStatus.RolledBack, actual.FinalStatus, "MySQL aborts transaction on error");
        ...
        [TestMethod]
        public async Task MySQL_RunBuildScripts_UnableToConnectToDatabase()
        {
            var init = GetInitializationObject();
            SqlSyncBuildDataModel buildData = init.CreateSqlSyncSqlBuildDataModelObject();
        ...
        [TestMethod]
        public async Task MySQL_RunBuildScripts_WithPreBatchedScripts()
        {
            var init = GetInitializationObject();
            SqlSyncBuildDataModel buildData = init.CreateSqlSyncSqlBuildDataModelObject();
        ...
        [TestMethod]
        public async Task MySQL_RunBuildScripts_AsScriptOnly()
        {
            var init = GetInitializationObject();
            SqlSyncBuildDataModel buildData = init.CreateSqlSyncSqlBuildDataModelObject();
            init.AddInsertScript(ref buildData, true);
        ...
        [TestMethod]
        public async Task MySQL_RunBuildScripts_InsertThenSelect_VerifyLogging()
        {
            var init = GetInitializationObject();
            SqlSyncBuildDataModel buildData = init.CreateSqlSyncSqlBuildDataModelObject();
            init.AddInsertScript(ref buildData, true);
        ...
        [TestMethod]
        [Ignore("Alternate logging DB with MySQL requires additional connection routing work")]
        public async Task MySQL_RunBuildScripts_AlternateLoggingDb()
        {
            var init = GetInitializationObject();
            SqlSyncBuildDataModel buildData = init.CreateSqlSyncSqlBuildDataModelObject();
            init.AddInsertScript(ref buildData, true);
        ...
        [TestMethod]
        public async Task MySQL_RunBuildScripts_NonTransactional_FinishCommitted()
        {
            var init = GetInitializationObject();
            SqlSyncBuildDataModel buildData = init.CreateSqlSyncSqlBuildDataModelObject();
            init.AddInsertScript(ref buildData, true);
        ...
        [TestMethod]
        public async Task MySQL_RunBuildScripts_NonTransactional_FinishCommittedWithScripting()
        {
            var init = GetInitializationObject();
            SqlSyncBuildDataModel buildData = init.CreateSqlSyncSqlBuildDataModelObject();
            init.AddInsertScript(ref buildData, true);
        ...
        [TestMethod]
        public async Task MySQL_RunBuildScripts_NonTransactional_Failure()
        {
            var init = GetInitializationObject();
            SqlSyncBuildDataModel buildData = init.CreateSqlSyncSqlBuildDataModelObject();
            init.AddInsertScript(ref buildData, true);
        ...
        [TestMethod]
        public async Task MySQL_RunBuildScripts_NonTransactional_WithFailureDontCauseFailure()
        {
            // Note: In non-transactional mode on MySQL, each script runs independently.
            // Even with CausesBuildFailure=false, MySQL's error propagation currently
            ...
            // MySQL non-transactional: successful script's data persists regardless
        ...
        [TestMethod]
        public async Task MySQL_RunBuildScripts_NonTransactional_WithFailureDontRollbackDontCauseFailure()
        {
            var init = GetInitializationObject();
            SqlSyncBuildDataModel buildData = init.CreateSqlSyncSqlBuildDataModelObject();
            init.AddInsertScript(ref buildData, true);
        ...
        [TestMethod]
        public async Task MySQL_LogTableExists_ReturnsTrue()
        {
            var init = GetInitializationObject();
        ...
        [TestMethod]
        public async Task MySQL_LogTableExists_InvalidDb_ReturnsFalse()
        {
            var init = GetInitializationObject();
        ...
        [TestMethod]
        public async Task MySQL_LogTableExists_NoDB_ReturnsFalse()
        {
            var init = GetInitializationObject();
            // Use the admin database "MySql" which shouldn't have our logging table
            var adminConnData = init.CreateConnectionData("MySql");
            var factory = ConnectionHelper.GetFactory(DatabasePlatform.MySQL);
        ...
        [TestMethod]
        public void MySQL_HasBlockingSqlLog_FindsExistingEntry()
        {
            var init = GetInitializationObject();
        ...
        [TestMethod]
        public void MySQL_HasBlockingSqlLog_DoesNotFindMissing()
        {
            var init = GetInitializationObject();
        ...
        [TestMethod]
        public void MySQL_HasBlockingSqlLog_InvalidDbName()
        {
            var init = GetInitializationObject();
        ...
        [TestMethod]
        public void MySQL_GetBlockingSqlLog_FindsAndDoesntFindEntries()
        {
            var init = GetInitializationObject();
            SqlSyncBuildDataModel buildData = init.CreateSqlSyncSqlBuildDataModelObject();
            init.AddPreRunScript(ref buildData, false);
        ...
        public void MySQL_ClearScriptBlocks()
        {
            if (!isMySqlAvailable)
                Assert.Inconclusive("MySQL is not available.");

            MySqlInitialization init = GetInitializationObject();
        ...
        [TestMethod]
        public void MySQL_RecordCommittedScripts()
        {
            if (!isMySqlAvailable)
                Assert.Inconclusive("MySQL is not available.");
        ...
        [TestMethod]
        public async Task MySQL_GetScriptRunLog_ReturnsEntries()
        {
            if (!isMySqlAvailable)
                Assert.Inconclusive("MySQL is not available.");
            ...
            Assert.IsTrue(actual.Count > 0, string.Format("Missing rows for pre-run script. {0}", MySqlInitialization.MissingDatabaseErrorMessage));
        ...
        [TestMethod]
        public void MySQL_GetScriptRunLog_InvalidDb_ThrowsException()
        {
            if (!isMySqlAvailable)
                Assert.Inconclusive("MySQL is not available.");
        ...
        [TestMethod]
        public async Task MySQL_SaveBuildDataSet_NullProjectFileName()
        {
            if (!isMySqlAvailable)
                Assert.Inconclusive("MySQL is not available.");
        ...
        [TestMethod]
        public async Task MySQL_SaveBuildDataSet_EmptyProjectFileName()
        {
            if (!isMySqlAvailable)
                Assert.Inconclusive("MySQL is not available.");
        ...
        [TestMethod]
        public async Task MySQL_SaveBuildDataSet_NullBuildHistoryName()
        {
            if (!isMySqlAvailable)
                Assert.Inconclusive("MySQL is not available.");
        ...
        [TestMethod]
        public async Task MySQL_SaveBuildDataSet_EmptyBuildHistoryName()
        {
            if (!isMySqlAvailable)
                Assert.Inconclusive("MySQL is not available.");
        ...
        [TestMethod]
        public async Task MySQL_ProcessBuild_SingleInsert_CommitsAndLogs()
        {
            if (!isMySqlAvailable)
                Assert.Inconclusive("MySQL is not available.");
        ...
        [TestMethod]
        public async Task MySQL_ProcessBuild_FailureCausesBuildFailure_RollsBack()
        {
            if (!isMySqlAvailable)
                Assert.Inconclusive("MySQL is not available.");
        ...
        [TestMethod]
        public async Task MySQL_ProcessBuild_MultipleInserts_AllCommit()
        {
            if (!isMySqlAvailable)
                Assert.Inconclusive("MySQL is not available.");
        ...
        [TestMethod]
        public async Task MySQL_ProcessBuild_AsMultiDbRun()
        {
            if (!isMySqlAvailable)
                Assert.Inconclusive("MySQL is not available.");
        ...
        [TestMethod]
        public async Task MySQL_ProcessBuild_NonTransactional_FailureThenSuccess()
        {
            if (!isMySqlAvailable)
                Assert.Inconclusive("MySQL is not available.");
        ...
        [TestMethod]
        public async Task MySQL_ProcessBuildAsync_MultipleScripts_AllCommitAndLog()
        {
            if (!isMySqlAvailable)
                Assert.Inconclusive("MySQL is not available.");
        ...
        [TestMethod]
        public async Task MySQL_ProcessBuildAsync_NonTransactional_PartialCommitOnFailure()
        {
            if (!isMySqlAvailable)
                Assert.Inconclusive("MySQL is not available.");
        ...
        [TestMethod]
        public async Task MySQL_ProcessBuildAsync_Trial_RollsBackWithNoDataChanges()
        {
            if (!isMySqlAvailable)
                Assert.Inconclusive("MySQL is not available.");
        ...
        [TestMethod]
        public async Task MySQL_ProcessBuildAsync_BatchedScripts_AllBatchesExecute()
        {
            if (!isMySqlAvailable)
                Assert.Inconclusive("MySQL is not available.");
