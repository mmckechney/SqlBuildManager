...
namespace SqlSync.TableScript
{
    ...
    public class DataDumpHelper
    {
        ...
        public void ExtractAndWriteData()
        {
            ...
            {
                if (ProcessingTableData != null)
                    ProcessingTableData(null, new ProcessingTableDataEventArgs(tableList[i], "Getting Column and Key Information", false));
            ...
            if (ProcessingTableData != null)
                ProcessingTableData(null, new ProcessingTableDataEventArgs("", "", true));
        ...
        private bool WriteData(string tableName, SqlCommand cmd, ColumnInfo[] columns, string header)
        {
            try
            {
                ...
                string fileName = Path.Combine(basePath, tableName + ".data");

                if (ProcessingTableData != null)
                    ProcessingTableData(null, new ProcessingTableDataEventArgs(tableName, "Writing Data to " + Path.GetFileName(fileName), false));
                ...
                if (FileWritten != null)
                {
                    ...
                    FileWritten(null, new FileWrittenEventArgs(fileName, size));
                }
                return true;
            }
            catch (Exception e)
