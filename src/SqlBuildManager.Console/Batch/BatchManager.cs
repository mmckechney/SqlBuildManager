    ...
    public class BatchManager
    {
        ...
        public async Task<(int retval, string readOnlySas)> StartBatch(bool stream = false, bool unittest = false)
        {
            ...
            try
            {
                ...
                var status = await PollBatchPoolAndNodeStatus(cmdLine, poolId, batchClient);
                if (status != 0)
                {
                    log.LogError("Batch pool failure.");
                    return (status, "");
                }
        ...
        public IList<string> CompileCommandLines(CommandLineArgs cmdLine, List<ResourceFile> inputFiles, string outputContainerUrl, int poolNodeCount, string jobId, OsType os, BatchType bType)
        {
            ...
            for (int i = 0; i < poolNodeCount; i++)
            {
                ...
                StringBuilder sb = new StringBuilder("/bin/sh -c 'cd /app && dotnet sbm.dll ");
        ...
        public async Task<int> PreStageBatchNodes()
        {
            ...
            bool success = await CreateBatchPool(cmdLine, PoolName);
            if (!success)
            {
        ...
        private async Task<int> PollBatchPoolAndNodeStatus(CommandLineArgs cmdLine, string poolName, BatchClient batchClient)
        {
            log.LogInformation($"Waiting for pool {poolName} to be created");
            var nodeCount = cmdLine.BatchArgs.BatchNodeCount;
            if (!cmdLine.BatchArgs.PollBatchPoolStatus) log.LogInformation($"--pollbatchpoolstatus flag set to 'false'. Pool is being created, but you will not get regular updates on the status. If you want to attach to pool to get status, you rerun the same command with '--pollbatchpoolstatus true' at any time.");
            ...
            while (true)
            {
                ...
                if (nodes.Where(n => n.State == ComputeNodeState.Unusable).Any())
                {
                    var badCount = nodes.Where(n => n.State == ComputeNodeState.Unusable).Count();
                ...
                if (nodes.Where(n => n.State != ComputeNodeState.Idle).Any())
                {
                    ...
                    else if (cmdLine.BatchArgs.PollBatchPoolStatus)
                    {
                ...
                else
                {
                    ...
                    if (nodes.Where(n => n.State == ComputeNodeState.Idle).Count() != nodeCount)
                    {
                        var badCount = nodeCount - nodes.Where(n => n.State == ComputeNodeState.Idle).Count();
        ...
        public async Task<int> CleanUpBatchNodes()
        {
            ...
            if (!cmdLine.BatchArgs.PollBatchPoolStatus) log.LogInformation($"--pollbatchpoolstatus flag set to 'false'. Pool is being deleted, but you will not get regular updates on the status. If you want to attach to pool to get status, you rerun the same command with '--pollbatchpoolstatus true' at any time.");
            ...
            try
            {
                ...
                var pool = batchClient.PoolOperations.GetPool(PoolName, null, null);
                var count = batchClient.PoolOperations.ListComputeNodes(PoolName, null, null).Count();
                while (pool != null && pool.State == PoolState.Deleting && count > 0)
                {
                    isPolling = true;
