                ...
                else if (property.PropertyType == typeof(CommandLineArgs.Authentication)) //Special case if Key Vault is specified
                {
                    if (obj is CommandLineArgs cmd)
                    {
                        ...
                        else
                        {
                            ...
                            if (cmd.AuthenticationArgs.TrustServerCertificate == true)
                            {
                                args.AddRange(new string[] { "--trustservercertificate", cmd.AuthenticationArgs.TrustServerCertificate.ToString().Quoted() });
                            }
                        }
                    }
                }
